...
#if defined(NOT_USE_SEMI) //不用多线程semi
void Sem_init(void *sem, int pshared, unsigned int value)
{
    // printf("not use semi init\n");
}
...
#else
void Sem_init(dispatch_semaphore_t *sem, int pshared, unsigned int value) 
{
    *sem = dispatch_semaphore_create(1); // init with value of 1
}
#endif
...
#else
void P(dispatch_semaphore_t *sem)  
{
...
#else
void V(dispatch_semaphore_t *sem)
{
