...
function mt:start()
    if self.state == state.INIT then
        ...
        self.co = coroutine.create(self.__update)
            ...
            if tids then
                for _,tid in ipairs(tids) do
                    ...
                    if tinfo and tinfo.func then
                        local co = coroutine.create(tinfo.func)
                        coroutine.resume(co, tinfo.args)
                    end
                    self.tbl[tid] = nil
                end
            end
...
--- 立即分发计时器
-- @return void
function mt:dispatch(tid)
    local tinfo = self.tbl[tid]
    if not tinfo then
        return false
    end
    if tinfo and tinfo.func then
        if self:delete(tid) then
            tinfo.func(tinfo.args)
        end
    end
    return true
end
