   ...
   -- integer reply
    elseif prefix == ':' then
        ...
        if not number then
            ...
            client.error('cannot parse '..tostring(data)..' as a numeric response.')
        end
...
local function create_connection(parameters)
    ...
    if parameters.scheme == 'unix' then
        perform_connection, socket = connect_unix, require('src.socket.unix')
        ...
        perform_connection, socket = connect_tcp, require('src.socket').tcp
    end
...
function redis.connect(...)
    ...
    if #args == 1 then
        if type(args[1]) == 'table' then
            ...
            local uri = require('src.socket.url')
