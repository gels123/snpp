...
function initDBConf:executeGlobalDataSql()
    gLog.i("initDBConf:executeGlobalDataSql begin")
    if dbconf.dbtype == "mongodb" then
    
    elseif dbconf.dbtype == "mysql" then
        local sql = io.readfile("sql/mysql/globaldata.sql")
        -- gLog.d("initDBConf:executeGlobalDataSql sql=", sql)
        local address = require("svrAddrMgr").getSvr(svrAddrMgr.gameDBSvr)
        local ret = skynet.call(address, "lua", "execute", sql)
        assert(ret and not ret.err, "initDBConf:executeGlobalDataSql error, ret="..table2string(ret))
    end
    gLog.i("initDBConf:executeGlobalDataSql end")
end
